...
{
    const char *p = buf;
    /* AICI functia continua sa scrie pana cand tot bufferul a fost trimis. */
    while (len) {
...
{
    char *p = buf;
    /* AICI functia continua sa citeasca pana cand tot bufferul a fost primit. */
    while (len) {
...
static solver_result_t solve_serial(int n, const double *A, const double *b)
{
    ...
    /* AICI se executa rezolvarea seriala prin LAPACKE_dgesv. */
...
{
    int nc=n+1;
    /* AICI incepe substitutia inversa pe matricea deja triunghiulara superior. */
    for (int i=n-1; i>=0; i--) {
        double s=aug[i*nc+n];
                         ...
                         int n, int band_start, int band_len)
{
    ...
    /* AICI workerul copil primeste de la parinte banda lui initiala de randuri. */
    if (read_all(rfd,aug,(size_t)(band_len*nc)*sizeof(double))!=0)
        { free(aug);free(piv); _exit(1); }

    /* Bucla de eliminare */
    /* AICI incepe bucla de eliminare Gauss pentru fiecare coloana pivot k. */
    for (int k=0; k<n; k++) {
        /* Trimite randul pivot in sus daca il detinem */
        /* Daca workerul curent detine randul pivot, AICI il trimite parintelui. */
        ...
        /* AICI workerul primeste pivotul broadcastat de parinte. */
        if (read_all(rfd,piv,(size_t)nc*sizeof(double))!=0)
            { free(aug);free(piv); _exit(1); }
        /* Eliminare */
        /* AICI fiecare worker elimina necunoscuta curenta din randurile aflate sub pivot. */
    ...
    /* AICI workerul trimite inapoi parintelui banda finala procesata. */
                             ...
                             int num_workers)
{
    ...
    /* AICI se decide ramura seriala: sistem mic sau un singur worker. */
    ...
    /* AICI se construieste matricea augmentata [A|b] in memoria parintelui. */
    ...
    /* AICI se initializeaza capetele pipe-urilor si PID-urile workerilor. */
    for (int w=0;w<W;w++) {
        down[w][0]=down[w][1]=-1;
        up[w][0]  =up[w][1]  =-1;
        pids[w]=-1;
    ...
    /* AICI se creeaza efectiv pipe-urile pentru comunicarea parinte-copil. */
    for (int w=0;w<W;w++) {
    ...
    /* AICI parintele face fork() pentru toti workerii. */
    for (int w=0;w<W;w++) {
        ...
        if (pids[w]==0) {
            ...
            /* AICI copilul intra in rutina worker_child() si nu se mai intoarce in codul parintelui. */
            worker_child(down[w][0], up[w][1], n, bs, bl);
            /* nu se intoarce niciodata */
        }
    }
    /* AICI parintele trimite fiecarui worker banda lui initiala de randuri. */
    for (int w=0;w<W;w++) {
        close(down[w][0]); down[w][0]=-1;
        close(up[w][1]);   up[w][1]  =-1;
    }
    /* AICI parintele colecteaza randurile finale de la toti workerii. */
    for (int w=0;w<W;w++) {
        int bs=w*band, bl=(w==W-1)?(n-bs):band;
...

    {
        ...
        /* AICI parintele coordoneaza fiecare pas k: primeste pivotul si apoi il difuzeaza tuturor workerilor. */
        for (int k=0;k<n;k++) {
            int ow=row_owner(k,band,W);

            /* Primeste pivotul de la detinator */
            /* AICI parintele primeste pivotul de la workerul care detine randul k. */
            ...
            /* AICI parintele asteapta terminarea tuturor proceselor copil. */
    for (int w=0;w<W;w++) {
                /* AICI parintele retransmite pivotul tuturor workerilor (broadcast). */
    ...
    /*  substitutie inversa  */
    {
        ...
        /* AICI se face substitutia inversa pentru a obtine vectorul final x. */
