...
static void read_system_from_stdin(int *out_n, double **out_A, double **out_b)
{
    ...
    /* AICI utilizatorul introduce liniile matricei A si elementele lui b. */
    for (int i = 0; i < n; i++) {
        printf("  Randul %d: ", i + 1);
        for (int j = 0; j < n; j++) {
...
/* AICI incepe varianta interactiva: utilizatorul introduce manual sistemul Ax=b. */
int main(int argc, char *argv[])
{
    ...
    int c;
    /* AICI se parseaza host-ul si portul serverului. */
    ...
    /* ---- Conectare ---------------------------------------------------- */
    /* AICI se creeaza conexiunea TCP spre server. */
    ...
    msgHeaderType h;
    msgIntType    m;
    /* AICI se face handshake-ul pentru obtinerea clientID-ului. */
    ...
    /* ---- OPR_SOLVE ---------------------------------------------------- */
    /* AICI se trimite catre server cererea de rezolvare pentru datele introduse manual. */
    h.opID = OPR_SOLVE;
    ...
    int     status = 0;
    double *x      = NULL;
    /* AICI clientul primeste si afiseaza rezultatul. */
