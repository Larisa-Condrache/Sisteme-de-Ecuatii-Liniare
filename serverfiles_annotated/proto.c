...
msgHeaderType peekMsgHeader(int sock)
{
    ...
    /* AICI se citeste doar headerul, fara consumarea mesajului din buffer (MSG_PEEK). */
...
int readSingleInt(int sock, msgIntType *m)
{
    ...
    /* AICI se citeste integral un mesaj care contine un singur intreg. */
...
int writeSingleInt(int sock, msgHeaderType h, int i)
{
    ...
    /* AICI se trimite un mesaj care contine header + un singur intreg. */
...
int readMultiInt(int sock, msgIntType *m1, msgIntType *m2)
{
    ...
    /* AICI se citeste integral un mesaj care contine doua numere intregi. */
    nb = recv(sock, &s, sizeof(s), MSG_WAITALL);
    if (nb <= 0) {
        m1->msg = m2->msg = -1;
        return -1;
...
int writeMultiInt(int sock, msgHeaderType h, int i1, int i2)
{
    ...
    /* AICI se trimite un mesaj care contine header + doua numere intregi. */
    nb = send(sock, &s, sizeof(s), 0);
    if (nb <= 0)
        return -1;
    return (int)nb;
...
int readSingleString(int sock, msgStringType *str)
{
    ...
    /* AICI se citeste mai intai lungimea stringului, pentru a sti cati octeti urmeaza. */
    ...
    /* AICI se aloca memorie exact pentru stringul primit + terminatorul nul. */
    ...
    /* AICI se citesc efectiv octetii stringului. */
...
int writeSingleString(int sock, msgHeaderType h, char *str)
{
    ...
    /* AICI se trimite mai intai lungimea stringului. */
    ...
    /* AICI se trimite continutul stringului propriu-zis. */
                      ...
                      int n, const double *A, const double *b)
{
    ...
    /* AICI clientul trimite headerul special pentru operatia SOLVE, inclusiv ordinul n. */
    nb = send(sock, &rh, sizeof(rh), 0);
    if (nb <= 0) return -1;

    /* AICI clientul trimite matricea A. */
    nb = send(sock, A, (size_t)(n * n) * sizeof(double), 0);
    if (nb <= 0) return -1;

    /* AICI clientul trimite vectorul b. */
...
int readSolveRequest(int sock, int *n, double **A, double **b)
{
    ...
    /* AICI serverul citeste headerul cererii SOLVE. */
    ...
    *n = (int)ntohl((uint32_t)rh.n);
    ...
    /* AICI serverul aloca memorie pentru matricea A si vectorul b ce vor fi primite. */
