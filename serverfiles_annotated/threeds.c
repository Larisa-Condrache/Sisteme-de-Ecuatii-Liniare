...
int main(int argc, char *argv[])
{
    ...
    /* AICI se citeste din mediul de executie calea catre fisierul de configurare. */
    ...
    int c;
    /* AICI incepe parsarea argumentelor din linia de comanda. */
    ...
    /* AICI se incarca fisierul de configurare de pe disc. */
    ...
    /* AICI valorile primite in CLI suprascriu configuratia citita din fisier. */
    ...
    /* AICI se ruleaza demonstratia separata cu fork() + pipe() + waitpid(). */
    demo_fork();
    ...
    /* AICI porneste thread-ul pentru serverul pe socket UNIX. */
    ...
    /* AICI porneste thread-ul pentru serverul TCP/INET. */
    if (pthread_create(&inetthr, NULL, inet_main, &iport) != 0) {
        perror("[main] pthread_create inet_main");
        return EXIT_FAILURE;
    }

    /* AICI porneste thread-ul pentru serverul SOAP. */
    ...
    /* AICI thread-ul principal asteapta terminarea thread-urilor server. */
